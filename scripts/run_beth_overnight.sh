...
#   tmux new -s beth
#   cd /path/to/sentinel_ebpf && uv run python scripts/run_detector_eval.py --run-all --pace fast

set -e
cd "$(dirname "$0")/.."
LOG="${1:-run_all.log}"
echo "BETH run-all started at $(date -Iseconds). Log: $LOG"
nohup uv run python scripts/run_detector_eval.py --run-all --pace fast >> "$LOG" 2>&1 &
