...
def _extract_network_group_features(evt: Any, requested: set[str]) -> dict[str, float]:
  out: dict[str, float] = {}
  ...
  family_val = 0
  family_requested = "net_socket_family_norm" in requested or "net_af_hash" in requested
  if family_requested:
    if syscall_display == "socket":
      ...
      if "INET6" in af or af == "10":
        ...
        family_val = 2
  if "net_socket_family_norm" in requested:
