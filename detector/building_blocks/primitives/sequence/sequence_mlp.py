...
from detector.building_blocks.primitives.sequence.sequence_context import SequenceFeatureMeta
...
class OnlineSequenceNextTokenMLP:
  """Online next-token predictor consuming generic sequence features."""
  ...
  @staticmethod
  def _meta_from_features(_features: dict[str, float], meta: SequenceFeatureMeta | None = None) -> SequenceFeatureMeta | None:
    return meta if isinstance(meta, SequenceFeatureMeta) else None

  def score_only_raw(self, features: dict[str, float], *, meta: SequenceFeatureMeta | None = None) -> float:
  ...
  def score_and_learn_raw(self, features: dict[str, float], *, meta: SequenceFeatureMeta | None = None) -> float:
    meta = self._meta_from_features(features, meta)
    if meta is None or not bool(meta.ready):
      return 0.0
    x = self._vectorize(features)
  ...
  def score_and_learn(self, features: dict[str, float], *, meta: SequenceFeatureMeta | None = None) -> tuple[float, float]:
  ...
  def set_state(self, state: dict[str, Any]) -> None:
    ...
    if state.get("optim") is not None:
      try:
        ...
        logger.warning("SequenceMLP: could not load optimizer state; using fresh optimizer")
