...
class TokenClassTable:
  ...
  def get_label_idx(self, name: str) -> int:
    """
    Get the label (equal to its index in the table) for a token.
    If the token is not in the table, add it and return the new index.
    """
...
class SequenceFeatureMeta(Meta):
  ready: bool
  target_id: int
  num_classes: int
